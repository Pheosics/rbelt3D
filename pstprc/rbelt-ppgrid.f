...
!      if ((i.eq.9).and.(j.eq.3).and.(k.eq.8).and.(tstep.eq.280)) then
...
c     returns status=1 if x0 is outside of grid or = to x(nx)
      ...
      if ((x0.lt.(x(1)-(x(2)-x(1)))).or.
     &    (x0.gt.(x(nx)+(x(nx)-x(nx-1))/2))) then
         status=1
         return
      endif

      if ((x0.ge.(x(1)-(x(2)-x(1)))).and.
     &    (x0.lt.x(1))) then
        grdpos0 = 1
        return
      endif

      if ((x0.le.(x(nx)+(x(nx)-x(nx-1))/2)).and.
     &    (x0.gt.x(nx))) then
        grdpos0 = nx
        return
      endif
...
c     returns status=1 if x0 is outside of grid or = to x(nx)
      ...
      if ((x0.lt.(x(1)-(x(2)-x(1))/2)).or.
     &    (x0.gt.(x(nx)+(x(nx)-x(nx-1))/2))) then
         status=1
         return
      endif

      if ((x0.ge.(x(1)-(x(2)-x(1))/2)).and.
     &    (x0.lt.x(1))) then
        grdpos = 1
        return
      endif
